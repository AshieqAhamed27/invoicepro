...
export default function Dashboard() {
  ...
  const maxTrend = Math.max(...(stats.trends.map(t => t.value)), 1000);
                  ...
                  {invoices.map((inv) => (
                    <tr key={inv._id} className="group hover:bg-white/[0.02] transition-colors">
                      <td className="px-10 py-6">
                        ...
                        <p className="text-[10px] font-bold text-zinc-700 uppercase tracking-[0.1em]">{inv.invoiceNumber} • {inv.clientEmail}</p>
                      </td>
                      <td className="px-10 py-6">
                        <span className={`px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${inv.status === 'paid' ? 'bg-emerald-400/5 text-emerald-400 border-emerald-400/10' : 'bg-yellow-400/5 text-yellow-500 border-yellow-400/10'}`}>
                          {inv.status}
                        </span>
                      </td>
                      <td className="px-10 py-6 text-right">
                        <p className="text-lg font-black text-white italic tracking-tighter">{formatCurrency(inv.amount)}</p>
                      </td>
                      <td className="px-10 py-6 text-right">
                        <div className="flex justify-end gap-3">
                          {inv.status !== 'paid' && (
                            <button
                              ...
                              className="p-3 rounded-xl bg-white/5 border border-white/5 text-zinc-500 hover:text-yellow-400 hover:bg-yellow-400/10 transition-all disabled:opacity-30"
                              title="Send Reminder Email"
                            >
                              <svg className={`h-5 w-5 ${sendingReminderId === inv._id ? 'animate-spin' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" /></svg>
                            </button>
                          )}
                          <Link 
                            to={`/invoice/${inv._id}`}
                            className="p-3 rounded-xl bg-white/5 border border-white/5 text-zinc-500 hover:text-white hover:bg-black transition-all"
                          >
