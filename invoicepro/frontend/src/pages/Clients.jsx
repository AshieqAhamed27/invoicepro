            ...
            {clients.map((client) => (
              ...
              >
                <div className="absolute top-0 right-0 p-6 opacity-[0.03] group-hover:opacity-[0.1] transition-opacity pointer-events-none grayscale">
                   <svg className="h-16 w-16" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 3c1.66 0 3 1.34 3 3s-1.34 3-3 3-3-1.34-3-3 1.34-3 3-3zm0 14.2c-2.5 0-4.71-1.28-6-3.22.03-1.99 4-3.08 6-3.08 1.99 0 5.97 1.09 6 3.08-1.29 1.94-3.5 3.22-6 3.22z" /></svg>
                </div>
                
                <h3 className="text-2xl font-black text-white italic truncate mb-1">{client.name || 'Anonymous'}</h3>
                <p className="text-sm font-bold text-emerald-400/60 lowercase tracking-tight mb-8">{client.email}</p>

                <div className="space-y-4">
                   <div className="flex justify-between items-center bg-white/[0.02] p-3 rounded-xl border border-white/5">
                      <span className="text-[10px] font-black uppercase tracking-widest text-zinc-600">Total Lifetime</span>
                      <span className="text-sm font-black text-white">₹ {Number(client.totalInvoiced).toLocaleString()}</span>
                   </div>
                   <div className="flex justify-between items-center p-3">
                      <span className="text-[10px] font-black uppercase tracking-widest text-zinc-600">Invoices</span>
                      <span className="text-sm font-black text-zinc-400">{client.invoiceCount} issued</span>
                   </div>
                   <div className="pt-4 border-t border-white/5">
                      <p className="text-[10px] font-black uppercase tracking-widest text-zinc-700">Last Encounter</p>
                      <p className="text-xs font-bold text-zinc-500 mt-1">{new Date(client.lastInvoiced).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}</p>
                   </div>
                </div>
              </div>
            ))}
