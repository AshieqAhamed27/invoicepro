...
export default function PublicInvoice() {
  ...
  const handleUploadScreenshot = async () => {
    ...
    try {
      ...
      setScreenshotStatus('Uploading and verifying via AI...');

      const formData = new FormData();
      formData.append('screenshot', screenshot);

      const res = await api.post(`/invoices/public/${id}/verify-screenshot`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      });

      setScreenshotStatus('Payment successfully verified by AI!');
      alert(res.data.message || 'Payment successfully verified by AI!');
      fetchInvoice();
    } catch (err) {
      console.error(err);
      setScreenshotStatus(err.response?.data?.message || 'Verification failed. Please make sure the screenshot is clear.');
    } finally {
          ...
          <div className="flex-1">
            {!invoiceMeta.isProposal && invoice.status === 'pending' && (
              <div className="rounded-xl border border-gray-100 bg-gray-50/50 p-6">
                ...
                {/* AI UPI Screenshot Verification block */}
                {invoice.currency === 'INR' && (
                  <div className="mt-6 border-t border-gray-200 pt-6 text-left">
                    <p className="text-xs font-black uppercase tracking-widest text-slate-500 mb-2">Paid via UPI / Bank Transfer?</p>
                    <p className="text-[11px] text-gray-500 mb-4 leading-relaxed">
                      Upload your payment confirmation screenshot (from GPay, PhonePe, Paytm, or your bank). Our AI will instantly verify it and mark this invoice as Paid.
                    </p>
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                      <label className="flex items-center justify-center px-4 py-2 border border-slate-300 rounded-lg text-xs font-bold text-slate-700 bg-white hover:bg-slate-50 cursor-pointer transition active:scale-95">
                      ...
                      {screenshot && (
                        <button
                          ...
                          disabled={verifyingScreenshot}
                          className="px-5 py-2 bg-emerald-500 text-white rounded-lg text-xs font-black uppercase tracking-widest hover:bg-emerald-600 transition active:scale-95 disabled:opacity-50"
                        >
                          {verifyingScreenshot ? 'Verifying...' : 'Verify Receipt'}
                        </button>
                      )}
                    </div>
                    {screenshotStatus && (
                      <p className={`mt-2 text-xs font-bold ${screenshotStatus.includes('failed') || screenshotStatus.includes('Error') ? 'text-red-500' : 'text-slate-750'}`}>
                        {screenshotStatus}
                      </p>
                    )}
                  </div>
